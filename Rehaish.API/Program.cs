...
builder.Services.AddScoped<IAddressService, AddressService>();    // Address service registration
...
builder.Services.AddSwaggerGen(c =>
{
    ...
    // Set custom API info (title, description, version)
    c.SwaggerDoc("v1", new OpenApiInfo
    {
        Title = "ریہائش API v1",
        Version = "v1",
        Description = "ریہائش (Rehaish) - Modern e-commerce API platform."
    });
...
using (var scope = app.Services.CreateScope())
{
    ...
    var oldAdmin = context.Users.FirstOrDefault(u => u.Email == "papaishere@gmail.com");
    if (oldAdmin != null)
    {
        context.Users.Remove(oldAdmin);
    }

    if (!context.Users.Any(u => u.Email == "itsadmin@gmail.com"))
    {
        var admin = new User
        {
            Name = "Super Admin",
            Email = "itsadmin@gmail.com",
            Role = "Admin"
        };
