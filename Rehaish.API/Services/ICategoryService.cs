...
namespace MyntraClone.API.Services
{
    public interface ICategoryService
    {
        ...
        // Hierarchy methods
        Task<IEnumerable<CategoryHierarchyDto>> GetHierarchyAsync(); // Get full category tree
        Task<IEnumerable<CategoryDto>> GetRootCategoriesAsync(); // Get only root categories (no parent)
        Task<IEnumerable<CategoryDto>> GetSubCategoriesAsync(int parentId); // Get subcategories of a parent
    }
}
